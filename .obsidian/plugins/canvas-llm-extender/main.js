...
function isBox(node) {
  return node !== null && typeof node === "object" && "x" in node && "y" in node && "width" in node && "height" in node;
}
...
function findEmptySpace(neighbor_node, node_to_fit, distance_between, updown, leftright) {
  ...
  if (leftright) {
    positions.push(
      { x: neighbor_node.x + neighbor_node.width + distance_between, y: neighbor_node.y },
...
function fitToText(node, width, textsizenode) {
  ...
  const lineHeight = parseFloat(compStyle.lineHeight) || 19;
...
function addNodeChild(node, nodetext) {
  ...
  const altorder = [{ updown: true, leftright: false, node }];
  ...
  altorder.push({ updown: false, leftright: true, node });
...
var APIError = class extends OpenAIError {
  ...
  static makeMessage(error, message) {
    return (error == null ? void 0 : error.message) ? typeof error.message === "string" ? error.message : JSON.stringify(error.message) : error ? JSON.stringify(error) : message || "status code (no body)";
  }
...
var APIClient = class {
  constructor({
    ...
    this.maxRetries = validatePositiveInteger("maxRetries", maxRetries != null ? maxRetries : MAX_RETRIES);
...
// src/openai-utils.ts
async function openai_get_reply(prompt, model, temperature, apiKey) {
...
var CanvasLLMExtendPluginSettingsTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian2.Setting(containerEl).setName("OpenAI API key").setDesc("API key for OpenAI").addText(
      (text) => text.setPlaceholder(DEFAULT_SETTINGS.apiKey).setValue(this.plugin.settings.apiKey).onChange(async (value) => {
        this.plugin.settings.apiKey = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian2.Setting(containerEl).setName("OpenAI model").setDesc("Model from OpenAI to use for text generation").addText(
      (text) => text.setPlaceholder(DEFAULT_SETTINGS.model).setValue(this.plugin.settings.model).onChange(async (value) => {
        this.plugin.settings.model = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian2.Setting(containerEl).setName("OpenAI temperature").setDesc("Temperature for OpenAI (how wild the answers are)").addSlider(
      (slider) => {
        slider.setValue(this.plugin.settings.temperature).setLimits(0, 2, 0.1).setDynamicTooltip().onChange(async (value) => {
          this.plugin.settings.temperature = value;
          await this.plugin.saveSettings();
        });
      }
    );
    new import_obsidian2.Setting(containerEl).setName("Prompt").setDesc("Prompt before graph data is provided").addTextArea(
...
// src/main.ts
var CanvasLLMExtendPlugin = class extends import_obsidian3.Plugin {
  async onload() {
    try {
...
`;
    d.incoming.forEach(
      (incoming) => getNodeNeighbours(incoming).outgoing.forEach(
        (sibling) => prompt += `Sibling: ${sibling.text}
`
      )
    );
